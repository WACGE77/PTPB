...
├── admin.py
├── apps.py
├── consumers.py         # WebSocket消费者（包含SSH和RDP）
├── models.py            # 数据模型
├── protocol.py          # 协议实现（包含SSH和RDP）
├── serialization.py     # 序列化器（包含SSH和RDP）
├── tests.py
├── urls.py              # WebSocket路由
├── utils.py
└── views.py
...
- **模型设计**: 合理使用ForeignKey, OneToOneField
- **权限控制**: 严格遵循最小权限原则
- **审计日志**: 记录所有RDP会话操作
- **错误处理**: 统一错误响应格式
- **性能优化**: 合理使用数据库索引
...
通过以上实现，我们可以在现有的堡垒机项目中集成Windows远程桌面功能，将RDP集成到terminal模块中，类似于SSH的实现方式。使用Apache Guacamole实现RDP协议解析与Web化远程桌面，无缝对接现有RBAC权限、资源管理、审计日志模块，遵循原有代码规范。
